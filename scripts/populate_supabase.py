...
def load_hrp_data():
    ...
    for _, row in df.iterrows():
        ...
        records.append({
            ...
            'internal_id': str(row.get('internalId', '')),
            'start_date': row.get('startDate'),
            'end_date': row.get('endDate'),
            'plan_version': str(row.get('planVersion', '')),
...
def load_hno_data(year):
    ...
    df = pd.read_csv(hno_file, skiprows=[1])
    
    records = []
    for _, row in df.iterrows():
        records.append({
            'country_iso3': str(row.get('Country ISO3', '')),
            'admin1_pcode': str(row.get('Admin 1 PCode', '')) if pd.notna(row.get('Admin 1 PCode')) else None,
            'admin1_name': str(row.get('Admin 1 Name', '')) if pd.notna(row.get('Admin 1 Name')) else None,
            'admin2_pcode': str(row.get('Admin 2 PCode', '')) if pd.notna(row.get('Admin 2 PCode')) else None,
            'admin2_name': str(row.get('Admin 2 Name', '')) if pd.notna(row.get('Admin 2 Name')) else None,
...
def calculate_country_metrics(year, hno_records, hrp_records, pop_records):
    ...
    for hno in hno_records:
        ...
        cluster = hno['cluster']
        category = hno['category']
        
        # Only use overall totals
        if cluster == 'ALL' and '-' not in category:
            ...
            country_needs[iso3]['in_need'] = max(
                country_needs[iso3]['in_need'],
                hno['in_need']
            )
            country_needs[iso3]['targeted'] = max(
                country_needs[iso3]['targeted'],
                hno['targeted']
            )
            country_needs[iso3]['population'] = max(
                country_needs[iso3]['population'],
                hno['population']
            )
...
def calculate_cluster_metrics(year, hno_records):
    ...
    for hno in hno_records:
        ...
        cluster_data[cluster]['countries'].add(hno['country_iso3'])
        cluster_data[cluster]['in_need'] += hno['in_need']
        cluster_data[cluster]['targeted'] += hno['targeted']
...
def calculate_country_cluster_gaps(year, hno_records):
    ...
    for hno in hno_records:
        ...
        gaps[key]['in_need'] += hno['in_need']
        gaps[key]['targeted'] += hno['targeted']
